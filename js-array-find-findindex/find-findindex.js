...
function findUserByUsername(usersArray, username) {
  return usersArray.find((e) => { return e['username'] === username });
}
...
function removeUser(usersArray, username) {
  const pivot = usersArray.findIndex((e) => { return e['username'] === username });
  console.log(pivot);
  if (pivot === -1) return undefined;
  const user = usersArray[pivot];
  usersArray.splice(pivot, 1);
  return user;
}